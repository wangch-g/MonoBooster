...
class KittiEvalOdom():
    ...
    def eval(self, gt_txt, pred_txt, result_txt, seq=None):
        # gt_dir: the directory of groundtruth poses txt
        # results_dir: the directory of predicted poses txt
        self.plot_path_dir = os.path.dirname(pred_txt) + "/plot_path"
        ...
        for i in range(len(ave_t_errs)):
            ...
            result_file.write('\n')
            result_file.write("Model: {}".format(pred_txt.rstrip().split('/')[-1].split('.')[0]))
            # result_file.write('-------------------------------------------\n')
            # result_file.write("Translational error (%): {0:.2f}\n".format(ave_t_errs[i]*100))
            # result_file.write("Rotational error (deg/100m): {0:.2f}\n".format(ave_r_errs[i]/np.pi*180*100))
            result_file.write("{0:>10.2f}|".format(ave_t_errs[i]*100))
            result_file.write("{0:>10.2f}|".format(ave_r_errs[i]/np.pi*180*100))
            # result_file.write('-------------------------------------------\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--result_txt', type=str, help=" ")
    parser.add_argument('--seq', type=str, help="sequences to be evaluated", default='09')
    args = parser.parse_args()

    eval_tool = KittiEvalOdom()
    eval_tool.eval(args.gt_txt, args.pred_txt, args.result_txt, seq=args.seq)
